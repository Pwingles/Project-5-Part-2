...
**Theoretical Analysis:** Unlike Heaps which scale at $O(N \log N)$, our Hash Table scales Linearly ($O(N)$). The graph confirms this linear trend (straight diagonal line), proving that the average cost per operation remains constant $O(1)$.

**Figure 2:** Average Probes vs N.
![Average Probes](probes_vs_n.png)
**Result:** When viewing into the LRU profiles I notice that the average probing between double hasing with compact vs single hashing with compaction that The av probes on the double is aobut 1/3 of the amount compared to single probing.

**Figure 3:** Occupancy (Tombstones).
![Occupancy](occupancy.png)
**Result:** wheN i veiew occupancy tried i can see that the tombstones for both hover below 15%.
...
## 6. Takeaway
In conclusion, while Double Hashing is mathematically superior for reducing probes (about 1/3 of Single Hashing), Single Hashing is practically faster for this LRU workload due to CPU cache efficiency. However, neither would work well without Compaction, which is the essential mechanism that keeps the table from filling up with tombstones. Making the probing process much faster.
